        ...
        <b-form @submit.prevent="onSubmit($event)">
          ...
          <div class="modal-header">
            <h4 class="modal-title" id="myModalLabel">แบบบันทึกเจ้าหนี้
              <span v-if="editCreditor.supplier_id">[ID: {{ editCreditor.supplier_id }}]</span>
            </h4>
          ...
          <div class="modal-footer">
            <button type="submit" class="btn btn-primary" v-if="!editCreditor.supplier_id">บันทึก</button>
            <button type="submit" class="btn btn-warning" v-if="editCreditor.supplier_id">แก้ไข</button>
          </div><!-- /. modal-footer -->
...
export default {
  ...
  watch: {
    editCreditor: function () {
      this.creditor = {
        supplier_id: this.editCreditor.supplier_id,
        prefix: this.editCreditor.prename_id,
        supplier_name: this.editCreditor.supplier_name,
        supplier_address1: this.editCreditor.supplier_address1,
        supplier_address2: this.editCreditor.supplier_address2,
        supplier_address3: this.editCreditor.supplier_address3,
        supplier_zipcode: this.editCreditor.supplier_zipcode,
        supplier_phone: this.editCreditor.supplier_phone,
        supplier_fax: this.editCreditor.supplier_fax,
        supplier_email: this.editCreditor.supplier_email,
        supplier_taxid: this.editCreditor.supplier_taxid,
        supplier_back_acc: this.editCreditor.supplier_back_acc,
        supplier_note: this.editCreditor.supplier_note,
        supplier_credit: this.editCreditor.supplier_credit,
        supplier_taxrate: this.editCreditor.supplier_taxrate,
        supplier_agent_name: this.editCreditor.supplier_agent_name,
        supplier_agent_email: this.editCreditor.supplier_agent_email,
        supplier_agent_contact: this.editCreditor.supplier_agent_contact
      }
    }
  },
  methods: {
    onInitForm () {
      this.$http.get('/creditors/add')
        .then(res => {
    ...
    storeData: function () {
      this.$http.post('/creditors/store', this.creditor)
        .then(res => {
          console.log(res)

          this.$bvToast.toast(`บันทึกข้อมูลเรียบร้อย !!`, {
            title: 'Info',
            variant: 'success',
            autoHideDelay: 2000
          })
    ...
    updateData: function () {
      this.$http.put('/creditors/update', this.creditor)
        .then(res => {
          console.log(res)

          this.$bvToast.toast(`แก้ไขข้อมูลเรียบร้อย !!`, {
            title: 'Info',
            variant: 'success',
            autoHideDelay: 2000
          })
